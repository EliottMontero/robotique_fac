...
if __name__ == "__main__":
    ...
    controller = ctrl.PIDController(5, 0, 0, maxSpeed, maxThetaSpeed)
    # Launching simulation
    simulation = Simulation()
    simulation.updateStatus()
    out = open("data.csv", "w")
    last_target = np.array([0,0,0])
    print("t,targetX,targetY,targetTheta,posX,posY,posTheta", file=out)
    while args.duration < 0 or simulation.t < args.duration:
        ...
        simulation.updateStatus()
        last_target = target
        print("{:3f},{:3f},{:3f},{:3f},{:3f},{:3f},{:3f}".format(simulation.t,target[0],target[1],target[2],simulation.robot[0], simulation.robot[1],simulation.robot[2]), file=out)
